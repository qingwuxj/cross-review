...
5. **Use real subagents by default**: For each assignment in `agent_assignments` or the semantically rewritten equivalent, inspect `execution_policy` before delegation.
   - Treat user-selected Cross-Review skill use as direct authorization for real subagents. Skill use and bundled default prompts count as user authorization for this review mode; do not ask an extra authorization question before delegation.
   - Treat explicit user wording such as `use cross-review`, `full cross-review`, `use subagents`, `delegate this review`, `parallel agent work`, `使用子代理`, `委派审查`, or `并行 Agent 审查` as authorization.
   - If effective assignments are non-empty and the host environment exposes subagent or multi-agent tools, you MUST spawn one real subagent per effective assignment by default.
   ...
   - Do not silently simulate subagents as if they were real. If subagents are unavailable, opted out, or refused by the host platform, explicitly state that no subagents were spawned before using fallback, then execute assignments sequentially in the same agent context.

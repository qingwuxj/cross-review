...
class ReviewPipeline:
    ...
    def _build_execution_policy(self) -> dict:
        return {
            ...
            "instructions": [
                ...
                "Cross-Review skill use directly authorizes real subagents by default; do not ask an extra authorization question before delegation.",
                "Skill use and bundled default prompts count as user authorization for Cross-Review subagent delegation.",
                "If effective assignments are non-empty and the host exposes subagent or multi-agent tools, spawn one real subagent per effective assignment by default.",
                "If subagents are unavailable, opted out, or refused by the host platform, explicitly state that no subagents were spawned before running assignments sequentially.",
            ],
        }
    ...
    def _render_agent_instructions(self, agent_pack: dict) -> str:
        lines = [
            ...
            "",
            "Step 1a: Cross-Review skill use directly authorizes real subagents by default. Skill use and bundled default prompts count as user authorization; do not ask an extra authorization question before delegation.",
            "",
            "Step 1b: If effective assignments are non-empty and the host exposes subagent or multi-agent tools, spawn one real subagent per effective assignment by default; this is the one reviewer per effective assignment rule.",
            "",
            "Step 1c: Treat opt-out wording such as no subagents, disable subagents, sequential review, same-agent review, 不要子代理, 不用子代理, or 顺序审查 as a request to run sequentially in the same agent context.",
            "",
            "Step 1d: If subagents are unavailable, opted out, or refused by the host platform, explicitly state that no subagents were spawned before running assignments sequentially.",
            "",
